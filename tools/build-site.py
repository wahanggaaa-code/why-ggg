...
@media (prefers-reduced-motion: reduce){
  .rail-pin{position:relative;height:auto;justify-content:flex-start;}
...
@media(max-height:520px){
  ...
  .rail-wrap{height:auto !important;}
  .rail{transform:none !important;width:100%;overflow-x:auto;overflow-y:hidden;scroll-snap-type:x mandatory;scrollbar-width:none;padding:10px 0 6px;}
  .rail::-webkit-scrollbar{display:none;}
...
def loader_html():
    return ('<noscript><style>#loader{display:none !important}</style></noscript>\n'
            '<div id="loader">\n  <div id="videoWrap">\n'
...
def js_mrw():
    # Scroll word reveal untuk judul manifesto — ditiru dari index.html cabang main.
    return r"""
  (function(){
    const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
      ...
      nodes.forEach(node=>{
        ...
        txt.split(/(\s+)/).forEach(part=>{
          if(/\s+/.test(part)) frag.appendChild(document.createTextNode(part));
            ...
            s.className='mrw'; s.textContent=part; s.style.opacity='0.15'; s.style.transition='opacity 0.12s';
            frag.appendChild(s);
          }
        });
        node.parentNode.replaceChild(frag, node);
      });
      const words = el.querySelectorAll('.mrw');
      const REST=0.15, SPAN=0.85, WIN=0.22;
      function onScroll(){
        const rect = section.getBoundingClientRect();
        const vh = window.innerHeight;
        const mob = window.innerWidth <= 900;
        const start = mob ? vh*0.85 : vh*0.75;
        const end   = mob ? vh*0.30 : (-section.offsetHeight + vh*0.25);
        const progress = Math.min(1, Math.max(0, (start - rect.top) / (start - end)));
        const count = words.length;
        words.forEach((w,i)=>{
          const s = count<=1 ? 0 : (i/(count-1))*SPAN;
          const e = Math.min(1, s+WIN);
          let op = progress<=s ? REST : progress>=e ? 1 : REST+(1-REST)*((progress-s)/(e-s));
          w.style.opacity = op;
        });
      }
    ...
    setup(document.getElementById('manifestoReveal'), document.getElementById('manifesto'));
  })();
"""
...
def loader_js():
    # Logika intro video — ditiru persis dari index.html cabang main (terbukti jalan di main).
    return r"""
  (function(){
    const video = document.getElementById('crystalVideo');
    const loader = document.getElementById('loader');
    if(!video || !loader) return;
    const prefersReduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    function trulyHideLoader(){
    ...
    function triggerOut(){
      if(loader.classList.contains('out') || loader.classList.contains('gone')) return;
      const wrap = document.getElementById('videoWrap');
      // STEP 1: video hilang dulu (0.6s fade+scale) — jangan ikut tirai
      if(wrap && !wrap.classList.contains('out')){
        wrap.classList.add('out');
        setTimeout(()=>{
          loader.classList.add('out');
          setTimeout(trulyHideLoader, 800);
        }, 650);
        return;
      }
      loader.classList.add('out');
    ...
    function handleMeta(){
      ...
      // dipercepat: video +-1.6s, total loading sekitar 3 detik (pola main)
      if(d && isFinite(d) && d>0) video.playbackRate = d/1.6;
      else video.playbackRate = 3.2;
      video.play().catch(()=>{});
    }
    function startLoader(){
      if(prefersReduced){ trulyHideLoader(); return; }
      loader.classList.remove('out','gone');
      const wrap = document.getElementById('videoWrap');
      if(wrap) wrap.classList.remove('out');
      ...
      video.load();
      video.play().catch(()=>{});
      clearTimeout(window._loaderFallback);
      window._loaderFallback = setTimeout(()=>{ if(loader.style.display!=='none') triggerOut(); }, 3000);
    }
    video.addEventListener('canplay', handleMeta);
    video.addEventListener('ended', triggerOut);
    video.addEventListener('error', triggerOut);
    loader.addEventListener('transitionend', (e)=>{
      if(e.propertyName==='transform' && loader.classList.contains('out')) trulyHideLoader();
    });
    startLoader();
  })();
"""
...
def home_page():
    ...
    main = ('<div id="main">\n  ' + grain_div() + '\n\n  ' + page_header('home') + '\n\n  '
            ...
            + footer_html() + '\n</div>')
    loader = loader_html()
    script = js_reveal() + js_scramble() + js_mrw() + loader_js() + rail_js()
